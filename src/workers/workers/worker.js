...
async function handleProcessChunk(payload) {
    ...
    try {
        // Process the chunk
        const summary = await processChunkInWorker(chunk, importOptions, orgCache, rateLimiter, checkpointDir);
    ...
    try {
        switch (msg.type) {
            ...
            case 'shutdown':
                ...
                break;
            default:
                console.warn(`[Worker ${workerData?.workerId ?? '?'}] Unknown message type:`, msg.type);
        }
    }
    catch (err) {
